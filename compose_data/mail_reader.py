...
def get_order_from_mail(order_raw_num):
    ...
    # получаем письмо
    result, data = mail.search(None, f'(SUBJECT "{order_raw_num} - spbkonvert.com")')
    ...
    if shipping_type == 'Самовывоз. 1-2 дня' or  shipping_type.startswith('DPD'):
    ...
    obj['ShippingCost'] = float(price_to_num(order_info.loc['Стоимость доставки:']))
